...
| Mise en ligne, `deploy/`, fiche Play Store, ou identité visuelle | `docs/deployment.md` + `rehearse.sh` vert + `../INFRASTRUCTURE.md` (une fois en ligne) ; fiche : `app/store/` (scène rejouable par `tools/store/seed_demo.py`, images par `make_store_assets.py`, réponses aux questionnaires dans `declarations-play.md` — Play les réexamine à chaque envoi) ; intro : la maquette `tools/mockups/agora_intro.html` fait foi, `gen_intro_jingle.py` et `intro_screen.dart` en sont les traductions — les trois `BATTUES` bougent ensemble |
