...
| 7 | Créneaux communs | ✅ « Trouver un créneau » : durée, période, heures, week-ends, membres requis ; `invisible` = libre (signalé), `busy` = pris, rdv du groupe = pris ; un appui propose le rdv · en attente : un rdv accepté dans un autre groupe rend-il occupé ? | ✅ |
